...
class Serverless {
	 ...
	 *  - {boolean} generateTests set to true to generate a snapshot unit test based off the execution 
	 */
	constructor(options) {
  ...
  load(configPath, opt, replace) {
	...
	getFunctionHandler(functionName) {
		let f = this.config.functions[functionName];
		if(f) {
			let dot = f.handler.lastIndexOf('.');
			return f.handler.substring(dot + 1);
		} else {
			throw new ItemNotFoundException(functionName);
		}
	}
